...
Last updated: 2026-05-06 (session-047 — `aae-orc-if85` shipped: SDK base-URL override (`SIDESTEP_BASE_URL` + `Client::with_base_url`) plus six wiremock integration tests in `tests/wiremock_endpoint.rs`. Closes Live-API-tests-pending note from B7 slice 5. The flag/env/config source signal from y7lq is now integration-tested end-to-end through the audit JSONL. Three follow-ons filed: `aae-orc-8mq8` (API-fetched aux for enrich), `aae-orc-u7hy` (pagination), `aae-orc-1mgo` (MissingParam should name the chain).

...
Last updated: 2026-05-06 (session-047 — three tickets shipped on the same day: `aae-orc-if85` (SDK base-URL override + 6 wiremock integration tests), `aae-orc-1mgo` (chain-naming error message replacing bare `MissingParam` for owner/customer per cli-philosophy.md), and `aae-orc-lyjy` (audited: items 1+2 done, item 3 deferred upstream). The val-resolution-chain arc from session-045 onward is now end-to-end: rule (cli-philosophy.md) → bedrock (val-resolution-chain) → first instantiation (token, B5) → second instantiation (owner/customer, B5 cross-link, finding-003) → integration harness (B8, finding-004) → ergonomic error (this commit, finding-005). 127 tests green.
...
First instantiation of the bedrock `val-resolution-chain` pattern
(promoted session-045, finding-002). Second instantiation lives
adjacent — owner/customer path params, shipped session-046,
`aae-orc-y7lq`, finding-003. The two share `auth::write_config`,
`Config`, `read_config`, and the audit-source recording shape.
...
### B8: Test Surface — wiremock Integration Harness

End-to-end coverage of the Client's HTTP layer via the
`SIDESTEP_BASE_URL` override (`Client::with_base_url` + the env-var
respected by `from_env`/`with_token`). Tests in
`crates/sidestep-cli/tests/wiremock_endpoint.rs` use `#[tokio::test]`
to spin up a `wiremock::MockServer`, mount a `Mock` with explicit
method/path/query/header expectations, then run the `sidestep` binary
synchronously via `assert_cmd` with the override env set. Mock
expectations are verified on `MockServer::drop` — failure raises a
panic with the actual requests received.
...
now integration-tested for all three sources (flag / env / config).

Evidence: `aae-orc-if85` (closed). Charter B7 slice 5's
"Live-API integration tests pending" note retired.
ADR: none — the override is testing infrastructure, not a
contract change.
...
| Chain-naming error for missing owner/customer (047, same day) | 2026-05-06 | `aae-orc-1mgo` shipped. New `check_required_chain_params(op_id, sources)` helper in CLI runs after `build_params` and before `call_op_blocking_for_verb` for each of `run_list` / `run_get` / `run_search`. When the operation's `path_params` declares a chain-tracked param (`owner` or `customer` per `CHAIN_PARAMS`) that didn't resolve through any layer of the chain, fail at the CLI with a four-source error message naming `--<param>`, `SIDESTEP_<PARAM>`, `sidestep auth login --<param>`, and `sidestep config set <param>` — closing the cli-philosophy.md "The fix" gap noted in finding-002. Bare `SidestepError::MissingParam` from the SDK still fires for non-chain-tracked params (`runid`, `repo`, `head_sha`, …) where the chain doesn't apply. Coverage: 3 new CLI integration tests in `tests/auth_chain.rs` (list rule no owner → owner-chain message with all four sources; list audit_log no customer → customer-chain message with all four sources; list rule with --owner flag → chain check passes, falls through to network-level failure). Charter B5 arc completed. 127 tests green; clippy + cargo deny + nightly fmt clean. Finding: `_kos/findings/finding-005-chain-naming-error.md`. bd: `aae-orc-1mgo` closed. |
| lyjy audit (047, same day) | 2026-05-06 | Audited `aae-orc-lyjy` (bd hooks fail in shared-server external mode). Items 1 (apply env-source fix to other four hooks) and 2 (subrepo audit) confirmed done — all five orc hooks already carry the workaround per session-042 commit a90ad1f, and an audit of the 15 subrepos with `.beads -> ../.beads` symlinks found zero subrepo bd-installed git hooks (none ran `bd init`). Item 3 (upstream-or-orc proper fix per finding-040: bd writes per-repo .env, hooks consult OS keyring, or hooks fall back to no-server partial export) deferred to a future session — needs a bd upstream issue filed at steveyegge/beads. Ticket notes updated; not closed. |
| Base-URL override + wiremock harness (047) | 2026-05-06 | `aae-orc-if85` shipped end-to-end. SDK: `pub const BASE_URL_ENV = "SIDESTEP_BASE_URL"` + `Client::with_base_url(token, base_url)` constructor + `from_env`/`with_token` honor the env var (empty string treated as unset). Trailing-slash trimmed on override path matching `spec.rs` loader. CLI: dev-dependency on `wiremock` added; new `tests/wiremock_endpoint.rs` with six `#[tokio::test]` integration tests covering: list with `--owner` flag, list with `SIDESTEP_OWNER` env, list with `[default] owner` config, list flag-overrides-env-and-config chain ordering, list bare-array response shape (vs wrapped), and get-by-id with multi-path-param endpoint. Each test mounts a wiremock `Mock` with method/path/query/header expectations, runs the binary via `assert_cmd` with the base-URL override + audit dir env, then verifies stdout records, audit JSONL `path_params_source.<param>` source, and the wiremock expectation count on drop. The y7lq audit signal is now integration-tested across all three sources — closing the loop noted in finding-003. 124 tests green (was 115); clippy + cargo deny + nightly fmt clean. Charter B8 added (testing surface), Last updated rolled to 2026-05-06. Three follow-ons filed: `aae-orc-8mq8` (API-fetched aux for `enrich --policies`), `aae-orc-u7hy` (pagination in `run_list`), `aae-orc-1mgo` (`MissingParam` error should name the chain per cli-philosophy.md). Finding: `_kos/findings/finding-004-base-url-override-and-wiremock-harness.md`. bd: `aae-orc-if85` closed; 3 filed. |
| Owner/customer chain shipped (046) | 2026-05-04 | `aae-orc-y7lq` end-to-end: SDK gains `ParamSource` (`flag` / `env` / `config`), `ResolvedParam`, `resolve_owner` / `resolve_customer`, `read_config` / `write_config(impl FnOnce(&mut Config))`. `Config` extended with `[default]` section (`owner`, `customer`); `[auth]` and `[default]` both serialize-skip when empty. `Span` gains `path_params_source: BTreeMap<String, ParamSource>` and `with_path_params_source` builder; `base_record` emits the field as a top-level sibling of `operation`. `CallOptions` threads it through `Client::call_op`. CLI: `auth login --owner / --customer` (token args optional when at least one is given); `auth status` reports owner + customer + source alongside token; new `config show / path / set <key> <value> / unset <key>` subcommand (keys: `owner`, `customer`, `auth.token`); `list / get / search` gain `--customer` symmetric with `--owner`; `build_params` now returns `(Value, BTreeMap<String, ParamSource>)` so call sites can pass the source map into `CallOptions`. Audit `schema_version` stays at 2 — additive within the source record (per the ticket scope). Coverage: 6 new SDK auth unit tests + 3 new SDK audit unit tests + 10 new CLI integration tests (`tests/auth_chain.rs`). 115 tests green; clippy + cargo deny + nightly fmt clean. Charter B5 cross-linked with the second instantiation; `Last updated` rolled. Finding: `_kos/findings/finding-003-owner-customer-resolution-shipped.md`. bd: `aae-orc-y7lq` closed. |
